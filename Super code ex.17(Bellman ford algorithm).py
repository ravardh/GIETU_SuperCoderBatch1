def bellman_ford(nodes, edges, start):
    # Create edge list with both source and destination lists
    edge_list = {node: ([], []) for node in nodes}
    for node1, node2, weight in edges:
        edge_list[node1][0].append((node2, weight))  # Outgoing edges
        edge_list[node2][1].append((node1, weight))  # Incoming edges

    distances = {node: float('inf') for node in nodes}
    distances[start] = 0

    # Iterate n-1 times over all edges
